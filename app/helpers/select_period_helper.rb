...
module SelectPeriodHelper
  ...
  def select_period_tag(period_type, period, caption, change_last_date=false)
    ...
    disabled = ""
    disabled = "disabled=\"true\"" if period_type != "other"
    period_string = content_tag(:span, "с ")
    period_string << ("<input type=\"text\" id=\"day\" name=\"day\" class=\"datepicker\"#{disabled} readonly=\"true\" value=\"#{period.first.strftime("%d.%m.%Y")}\">").html_safe
    if change_last_date
      period_string << content_tag(:span, " по ")
      period_string << ("<input type=\"text\" id=\"day2\" name=\"day2\" class=\"datepicker\"#{disabled} readonly=\"true\" value=\"#{period.last.strftime("%d.%m.%Y")}\">").html_safe
    else
